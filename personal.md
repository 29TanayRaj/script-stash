...
## Overview
The **RAG + LLM Powered Chatbot** combines **Retrieval-Augmented Generation (RAG)** with Large Language Models (LLMs) to provide a context-aware conversational interface. The system enables users to interact with the chatbot and leverage contextual information from uploaded documents for accurate and dynamic responses.
...
3. Fiass - For storing the vector embeddings of the documents (chuncks).
4. OllamaEmbeddings - For generating embeddings for the vector Database.
5. PyPDFLoader - For
6. Pickle Library- For saving the 
7. Streamlit - For front end of the system. 
...
- Need fot this custom class arose while creating of the fallback, “Sorry, I didn’t understand your question. Do you want to connect with a live agent?”.
...
- The was to fix this was to normalize the vectors in the embeddings generated by the 
...
- **AI Responses**: Fetches answers using RAG or directly from the LLM based on the database's availability.
...
1. **Database-Enhanced Responses**:
   ...
   - Filters results with a relevance score > 0.2.
...
## Notes
- Ensure the LLM model selected is compatible with your hardware and setup.
- Use normalized embeddings for consistent similarity scoring.

For more details or troubleshooting, refer to the code or contact the development team.
